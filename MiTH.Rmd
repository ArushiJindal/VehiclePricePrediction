...
test = read.csv("new_test.csv",header=TRUE,na.strings = c(""," ","?","NA",NA),stringsAsFactors = FALSE)
valid = read.csv("test.csv",header=TRUE,na.strings = c(""," ","?","NA",NA),stringsAsFactors = FALSE)
...
#barchart(train$NameOfTheVehicle)
...
boxplot(data$PowerOfTheEngine) ## Too many outliers in this attribute, this will be dealt with later
...
numCorr <- findCorrelation(cor(num_data), cutoff = .80)
