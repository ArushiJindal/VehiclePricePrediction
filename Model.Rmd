...
factCols = c("DataCollectedDate","VehicleType","YearOfVehicleRegistration","GearBoxType","ModelOfTheVehicle","MonthOfVehicleRegistration","TypeOfTheFuelUsed","BrandOfTheVehicle","IsDamageRepaired","DateOfAdCreation","DateOfAdLastSeen","VehicleName")
...
fit.rf = randomForest(Price ~ ., data=d_train, keep.forest=TRUE, ntree=70, mtry = 40)
...
```
### Improve the random forest with tuning

```{r}

control <- trainControl(method="repeatedcv",number=5,repeats=2,allowParallel = TRUE,savePredictions = TRUE,verboseIter = TRUE)

grid <-  expand.grid(mtry = 11)

fit.rf1 = caret::train(Price ~ ., data=d_train, method = 'rf',  metric = "RMSE",trControl = control, tuneLength=30,tuneGrid=NULL, search = 'random')
...
rf1.train.pred <- predict(fit.rf1,d_train)
rf1.valid.pred <- predict(fit.rf1,d_train)

regr.eval(rf1.train.pred,d_train$Price)
regr.eval(rf1.test.pred,d_train$Price)
